  ...
  return (
    <div className="bg-white p-3 rounded-xl border border-gray-200 shadow-sm flex flex-col w-full h-full min-h-[240px] md:min-h-0 overflow-hidden">
      ...
      <div className="flex justify-between items-start mb-2 flex-none gap-2">
        ...
        <button className="flex items-center gap-2 px-3 py-1.5 bg-white border border-gray-200 rounded-lg text-xs text-gray-500 hover:border-emerald-500">
           <span className="hidden sm:inline">Filter</span> <Calendar className="w-3 h-3 text-gray-400" />
        </button>
      </div>

      <div className="flex-1 flex flex-col items-center justify-center relative min-h-0 w-full">
        <svg viewBox="0 0 300 180" className="w-full h-full max-h-[200px] overflow-visible preserve-3d">
          ...
          {[0, 20, 40, 60, 80, 100, 120, 140, 160, 180, 200].map((val) => {
             ...
             return (<text key={val} x={coords.x} y={coords.y} className="text-[18px] fill-gray-400 font-semibold font-sans" textAnchor="middle" dominantBaseline="middle">{val}</text>);
          })}

          <g transform={`translate(${cx}, ${cy}) rotate(${needleAngle - 90})`}> 
            <circle r="3" fill="#1f2937" /> <path d="M -2 0 L 2 0 L 0 -95 Z" fill="#374151" />
          </g>

          <text x={cx} y={cy + 30} textAnchor="middle" className="fill-gray-800 font-extrabold text-lg tracking-tight">80 km/h</text>

          <g transform={`translate(${cx - 100}, ${cy + 45})`}>
            <rect x="0" y="0" width="8" height="8" rx="2" fill="#10b981" />
            <text x="12" y="5" dominantBaseline="middle" className="text-[12px] fill-gray-500 font-bold">Lorem</text>
            <rect x="70" y="0" width="8" height="8" rx="2" fill="#f59e0b" />
            <text x="82" y="5" dominantBaseline="middle" className="text-[12px] fill-gray-500 font-bold">Lorem</text>
            <rect x="140" y="0" width="8" height="8" rx="2" fill="#ef4444" />
            <text x="152" y="5" dominantBaseline="middle" className="text-[12px] fill-gray-500 font-bold">Lorem</text>
          </g>
        </svg>
      </div>
    </div>
